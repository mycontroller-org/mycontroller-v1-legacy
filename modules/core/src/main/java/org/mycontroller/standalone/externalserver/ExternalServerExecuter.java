        ...
        if (resource.getExternalServersObject() != null) {
            for (ExternalServerTable extServer : resource.getExternalServersObject()) {
                if (extServer.getEnabled()) {
                    IExternalServerDriver _driver = ExternalServerFactory.getDriver(extServer.getId());
                    try {
                        if (_driver != null) {
                            _driver.write(sensorVariable);
                        }
                    } catch (Exception ex) {
                        _logger.error("Exception when sending data to server: {}, ", _driver.config(), ex);
                    }
                }
            }
        }
