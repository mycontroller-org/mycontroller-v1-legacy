...
public class GatewayMQTT extends GatewayAbstract {
    ...
    public GatewayMQTT(GatewayTable gatewayTable, IMessageParser<MessageMQTT> parser, IQueue<IMessage> queue) {
        super(new GatewayConfigMQTT(gatewayTable));
        _config = (GatewayConfigMQTT) config();
        _driver = new MQTTDriver(_config, parser, queue);
    }
