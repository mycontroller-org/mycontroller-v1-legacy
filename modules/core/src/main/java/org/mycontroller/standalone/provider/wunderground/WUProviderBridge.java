 ...
 */
@Slf4j
public class WUProviderBridge implements IProviderBridge {

    public WUProviderBridge() {
    }

    @Override
    public void executeMcMessage(McMessage mcMessage) {
        if (mcMessage.getNetworkType() != NETWORK_TYPE.WUNDERGROUND) {
            _logger.error("This is not '{}' message! McMessage:{}", NETWORK_TYPE.WUNDERGROUND.getText(), mcMessage);
            return;
        }
        try {
            _logger.debug("McMessage about to send to gateway: [{}]", mcMessage);
            McMessageUtils.sendToGateway(new WURawMessage(mcMessage).getRawMessage());
    ...
    public void executeRawMessage(RawMessage rawMessage) {
        if (rawMessage.getNetworkType() != NETWORK_TYPE.WUNDERGROUND) {
            _logger.error("This is not '{}' message! RawMessage:{}", NETWORK_TYPE.WUNDERGROUND.getText(), rawMessage);
        }
        try {
            _logger.debug("Received raw message: [{}]", rawMessage);
            McMessage mcMessage = new WURawMessage(rawMessage).getMcMessage();
    ...
    public RawMessage getRawMessage(McMessage mcMessage) throws RawMessageException {
        return new WURawMessage(mcMessage).getRawMessage();
    }
